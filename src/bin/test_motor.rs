...
async fn main(_spawner: Spawner) {
    ...
    loop {
        ...
        info!("button pressed, outputting 10% duty cycle");
        set_motor_speed(&mut pwm, Channel::Ch2, 0.1);
        button.wait_for_falling_edge().await;
        info!("button released, outputting 40% duty cycle");
        set_motor_speed(&mut pwm, Channel::Ch2, 0.4);
        button.wait_for_falling_edge().await;
        info!("button pressed, outputting 70% duty cycle");
        set_motor_speed(&mut pwm, Channel::Ch2, 0.7);
        button.wait_for_falling_edge().await;
        info!("button released, outputting 100% duty cycle");
